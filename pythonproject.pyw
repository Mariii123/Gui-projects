...
def bills():
   ...
   def total():
       ...
       var11.set(subtot)
       tax=round(var1.get()*10*0.05+var2.get()*20*0.05+var3.get()*2*0.050+var4.get()*3*0.050+var5.get()*15*0.05+var6.get()*75*0.05+var7.get()*50*0.05+var8.get()*40*0.05+var9.get()*30*0.05+var10.get()*35*0.05,2)
       var13.set(tax)
   ...
   r5=Label(lf1a,text="15 Rs/-",font=('Times New Roman',20,'bold'),fg='brown2')
   r5.grid(row=5,column=2,padx=10,pady=10,sticky=W)
   r6=Label(lf1b,text="75 Rs/-",font=('Times New Roman',20,'bold'),fg='brown2')
...
def getinfo():
   ...
   def get():
       ...
       txt.configure(state='normal')
       txt.delete('1.0',END)
       c.execute('SELECT * FROM data2 where bill_no='+str(b))
       d=c.fetchall()
       if len(d)>0:       
          for i in d:
              txt.insert("1.0","\t A-Z  IceCream \t \n ")
...
que.pack()
billbtn=Button(root,bg='powder blue',font=('georgia',15),cursor='hand2',text="Billing System",relief='ridge',bd=10,width=40,height=10,command=bills)
billbtn.place(x=60,y=200)
billbtn.bind('<Enter>',billcolor1)
billbtn.bind('<Leave>',billcolor2)
recbtn=Button(root,bg='powder blue',font=('georgia',15),cursor='hand2',text="Retreival System",relief='ridge',bd=10,width=40,height=10,command=getinfo)
recbtn.place(x=740,y=200)
recbtn.bind('<Enter>',reccolor1)
recbtn.bind('<Leave>',reccolor2)
ex=Button(root,bg='powder blue',font=('georgia',15),cursor='hand2',text="Exit System",relief='ridge',bd=10,width=40,height=5,command=exit)
